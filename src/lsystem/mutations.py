...
def remove_branch(transformations, key, index, is_open_bracket):
    ...
    :param transformations: the set of rules
    :param key: the key of the rule that is currently being altered
    :param index: index of the branch character
    :param is_open_bracket: True if char at transformations[key][index] is '[', False if ']'
    """
    list_conversion = list(transformations[key])
    ...
    for idx, ch in enumerate(loop_list):
        bracket_counter += 1 if ch == '[' else 0  # should +1 at index_left at begin of for-loop, so counter starts at 1
...
def add_branch(transformations, key, index_left):
    ...
    :param transformations: the set of rules
    :param key: the key of the rule that is currently being altered
    :param index_left: starting index of the new branch
    """
...
def remove_plus_minus(transformations, key, index):
    ...
    :param transformations: the set of rules
    :param key: the key of the rule that is currently being altered
    :param index: index of the +/- character in the rule transformations[key]
    """
    list_conversion = list(transformations[key])
...
def add_plus_minus(transformations, key, index):
    """Inserts a '+' or '-' (randomly chosen) to the rule transformations[key] at position 'index'
